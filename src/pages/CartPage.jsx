  ...
  const checkout = async (event) => {
    ...
    try {
      ...
      if (paymentMethod === "payway") {
        const payment = await createPayWayQrPayment(order.id);
        clearCart();
      ...
      <div className="cart-layout">
        ...
        <form className="order-summary" onSubmit={checkout}>
          ...
          <fieldset className="payment-methods">
            ...
            <label className={paymentMethod === "payway" ? "selected" : ""}><input type="radio" name="payment_method" value="payway" checked={paymentMethod === "payway"} onChange={() => setPaymentMethod("payway")} /><span><b>ABA PayWay KHQR</b><small>Scan inside ABA or another KHQR banking app</small></span></label>
          </fieldset>
          {error && <p className="form-error" role="alert">{error}</p>}
          <button className="primary-button wide" disabled={submitting}>{submitting ? "Preparing payment…" : paymentMethod === "payway" ? "Create ABA PayWay QR →" : "Pay with Bakong →"}</button>
          <p>{paymentMethod === "payway" ? "Secure KHQR generated by ABA PayWay" : "Secure Bakong KHQR checkout"}</p>
        </form>
      </div>
    </div>
  );
